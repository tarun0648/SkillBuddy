...
def analyze_resume_quality(resume_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if personal_info.get('name'):
            quality_score += 10
            strengths.append("Complete personal information")
        else:
            feedback.append("Missing name")
            weaknesses.append("Missing name in personal information")
        ...
        if personal_info.get('phone'):
            ...
            weaknesses.append("Missing phone number for direct contact")
        ...
        if work_experience:
            ...
            if len(work_experience) >= 2:
                strengths.append(f"Strong work experience with {len(work_experience)} positions")
            else:
        ...
        if skills:
            ...
            if len(skills) >= 10:
                strengths.append(f"Comprehensive skill set with {len(skills)} skills")
            elif len(skills) >= 5:
        ...
        if projects:
            ...
            if len(projects) >= 3:
                strengths.append(f"Strong project portfolio with {len(projects)} projects")
            else:
                strengths.append("Project experience included")
        else:
            weaknesses.append("No project portfolio included")
        
        # Check certifications
        certifications = resume_data.get('certifications', [])
        if certifications:
            strengths.append(f"Professional certifications included ({len(certifications)} total)")
        else:
        ...
        if awards:
            strengths.append(f"Recognition and awards included ({len(awards)} total)")
        else:
            weaknesses.append("No awards or recognition listed")
        
        # Additional analysis for strengths and weaknesses
        # Check for quantifiable achievements
        has_quantifiable_achievements = False
        for exp in work_experience:
            if exp.get('description'):
                desc = exp['description'].lower()
                if any(word in desc for word in ['%', 'percent', 'increased', 'decreased', 'improved', 'reduced', 'achieved', 'delivered']):
        ...
        for exp in work_experience:
            ...
            if end_year == 'Present' or (end_year.isdigit() and int(end_year) >= current_year - 2):
                recent_experience = True
                break
        
        if recent_experience:
            strengths.append("Recent work experience included")
        else:
            weaknesses.append("May need more recent work experience")
