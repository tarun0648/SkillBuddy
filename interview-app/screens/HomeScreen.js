...
export default function HomeScreen() {
  ...
  const { xpProgressPercentage, currentLevel, totalXP, loadXPData, error: xpError } = useXPStore();
  ...
  const lastXPFetchRef = useRef(0); // Track last XP fetch timestamp
  const XP_FETCH_COOLDOWN = 5 * 60 * 1000; // 5 minutes in ms
  ...
  // Load both XP and profile completion on mount
  useEffect(() => {
    const loadDataWithCooldown = async () => {
      const now = Date.now();
      const timeSinceLastFetch = now - lastXPFetchRef.current;
      
      // Only fetch XP if enough time has passed (5 minutes)
      if (timeSinceLastFetch >= XP_FETCH_COOLDOWN) {
        await loadXPData();
        lastXPFetchRef.current = now;
      } else {
        console.log('XP fetch skipped due to cooldown. Time remaining:', 
          Math.ceil((XP_FETCH_COOLDOWN - timeSinceLastFetch) / 1000 / 60), 'minutes');
      }
      
      // Profile completion can be fetched more frequently
      await loadProgressData();
    };
    
    loadDataWithCooldown();
  }, []);

  // Also refresh both when HomeScreen is focused, but with cooldown
  useFocusEffect(
    React.useCallback(() => {
      const loadDataWithCooldown = async () => {
        const now = Date.now();
  ...
  // Optional: Add a manual refresh button for XP
  const handleRefreshXP = async () => {
    const now = Date.now();
    const timeSinceLastFetch = now - lastXPFetchRef.current;
    
    if (timeSinceLastFetch < XP_FETCH_COOLDOWN) {
      const remainingMinutes = Math.ceil((XP_FETCH_COOLDOWN - timeSinceLastFetch) / 1000 / 60);
      Alert.alert(
        'Rate Limited', 
        `Please wait ${remainingMinutes} more minute(s) before refreshing XP again.`,
        [{ text: 'OK' }]
      );
      return;
    }
    ...
    try {
      await loadXPData();
      lastXPFetchRef.current = now;
      Alert.alert('XP Refreshed', 'XP data has been refreshed.');
    } catch (error) {
      Alert.alert('Error', 'Failed to refresh XP.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    if (inputValue.trim()) {
      ...
      const validateSocialUrl = (type, url) => {
        ...
        if (type === 'GitHub') {
          ...
          return githubRegex.test(urlToValidate);
        }
      ...
      if (!validateSocialUrl(currentInputType, formattedUrl)) {
        Alert.alert('Invalid URL', `Please enter a valid ${currentInputType} URL.`);
        return;
      }
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Progress Section */}
        {(() => {
          console.log('HomeScreen: Checking progress section conditions');
          console.log('HomeScreen: isAuthenticated:', isAuthenticated);
          console.log('HomeScreen: user:', user);
          console.log('HomeScreen: profileCompletionPercentage:', profileCompletionPercentage);
          console.log('HomeScreen: totalXP:', totalXP);
          console.log('HomeScreen: level:', currentLevel);
          
          // Show progress section if authenticated and user exists
              ...
              <View style={styles.progressSection}>
                ...
                <View style={styles.progressCards}>
                  <View style={styles.progressCard}>
                    ...
                    <ProgressBar percent={profileCompletionPercentage || 0} />
                    <Text style={styles.progressValue}>{profileCompletionPercentage || 0}%</Text>
                  </View>
                  <View style={styles.progressCard}>
                    <Text style={styles.progressLabel}>Level {currentLevel || 1}</Text>
                    <ProgressBar percent={xpProgressPercentage || 0} />
                    <Text style={styles.progressValue}>{totalXP || 0} XP</Text>
                    {xpError && xpError.includes('Rate limit exceeded') && (
                      <Text style={styles.cacheIndicator}>📱 Cached</Text>
                    )}
                  </View>
                </View>
              </View>
              ...
              <View style={styles.progressCards}>
                <View style={styles.progressCard}>
                  ...
                  <Text style={styles.loadingText}>
                    {isAuthenticated ? 'Loading...' : 'Please login to see your progress'}
                  </Text>
                </View>
                <View style={styles.progressCard}>
                  <Text style={styles.progressLabel}>Level</Text>
                  <Text style={styles.loadingText}>
                    {isAuthenticated ? 'Loading...' : 'Please login to see your XP'}
                  </Text>
                </View>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    padding: 20,
    paddingTop: 30,
    paddingBottom: 40, // Reduced padding since nav bar is removed
  },
  header: { 
    marginBottom: 30,
    alignItems: 'center',
